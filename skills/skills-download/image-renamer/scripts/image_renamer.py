...
"""
图片智能重命名工具
使用 OCR 识别图片内容，提取时间和金额等关键字进行重命名
"""
...
try:
    ...
    print("⚠ PaddleOCR 未安装，将使用简单的文件哈希命名")
except Exception as e:
    print(f"⚠ PaddleOCR 加载失败: {e}，将使用简单的文件哈希命名")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("用法:")
        print("  python image_renamer.py <目录路径> [--dry-run]")
        print("  python image_renamer.py --test")
        return
