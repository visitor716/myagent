...
**Symptoms**: `install-service` fails with `Failed to connect to bus`, `systemd --user is not available`, or `systemd user session not available in WSL2`.

**What it means**:

- The current WSL distro session does not expose a working `systemd --user` bus.
- Retrying `systemctl --user ...` in the same session will not fix it.

**What to do**:

1. Use `/claude-to-im start` instead of `install-service`
2. If `tmux` is installed, the Linux helper will prefer a detached `tmux` session for persistence
3. If you specifically need login-time auto-start, enable WSL systemd in `/etc/wsl.conf`:
   ```ini
   [boot]
   systemd=true
   ```
4. Restart WSL from Windows with `wsl --shutdown`, then start a new distro session and retry `install-service`

**Sandboxed agent note**:

- In sandboxed shells, PID probes or `tmux` / `systemctl` access can be denied even when the bridge is actually up.
- If `status` looks wrong but logs show a successful startup, trust the logs and rerun the management command with elevated permissions.
