...
On Linux, the helper prefers `systemd --user` when available, then `tmux` in WSL / non-systemd sessions, then `setsid` / `nohup`.
...
If stop reports that the current shell could not signal the process, rerun it from a less restricted shell or with elevated permissions instead of manually deleting PID files.
...
In sandboxed or restricted shells, process probing can return false negatives. If status says the process could not be verified but logs show a clean startup, confirm with `/claude-to-im logs` or rerun status with elevated permissions.
...
- Linux supervisor path (`systemd --user`, WSL `tmux` fallback, or `setsid/nohup`)
