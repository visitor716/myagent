...
export function loadConfig(): Config {
  ...
  return {
    ...
    tgReplyToMessage: env.has("CTI_TG_REPLY_TO_MESSAGE")
      ? env.get("CTI_TG_REPLY_TO_MESSAGE") === "true"
      : false,
...
export function saveConfig(config: Config): void {
  ...
  if (config.tgReplyToMessage !== undefined)
    out += formatEnvLine("CTI_TG_REPLY_TO_MESSAGE", String(config.tgReplyToMessage));
...
export function configToSettings(config: Config): Map<string, string> {
  ...
  m.set(
    "bridge_telegram_reply_to_message_enabled",
    config.tgReplyToMessage === true ? "true" : "false",
  );
