...
如果你希望桥接随 Linux 用户会话自动启动，可以安装 `systemd --user` 单元：
...
这样仍然沿用同一套 `daemon.sh` 的 start/stop/status 接口，但 Linux 会从 `setsid/nohup` 回退模式升级为带失败自动拉起的正式服务管理。
...
| `/claude-to-im install-service` | "开机自启" / "自启动" | 安装并启用 Linux 用户级自启动服务 |
| `/claude-to-im uninstall-service` | "关闭自启动" / "取消自启" | 移除 Linux 用户级自启动服务 |
