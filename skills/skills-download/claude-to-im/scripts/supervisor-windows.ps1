...
function Mask-SecretValue {
    param([string]$Value)
    if ([string]::IsNullOrEmpty($Value) -or $Value.Length -le 4) { return '****' }
...
function Mask-Line {
    ...
    $masked = [regex]::Replace(
        $Line,
        '(?i)((?:token|secret|password|api[_-]?key|auth[_-]?token)["'']?\s*[:=]\s*["'']?)([^\s"'']+)',
        { param($m) $m.Groups[1].Value + (Mask-SecretValue $m.Groups[2].Value) }
    )
    $masked = [regex]::Replace(
        $masked,
...
function Show-FailureHelp {
    ...
    if (Test-Path $LogFile) {
        Get-Content $LogFile -Tail 20 | ForEach-Object { Mask-Line $_ }
    } else {
    ...
    if (Test-Path $ErrLogFile) {
        Get-Content $ErrLogFile -Tail 20 | ForEach-Object { Mask-Line $_ }
    } else {
