...
PASS=0
FAIL=0
TMUX_SESSION="${CTI_TMUX_SESSION:-claude-to-im-bridge}"
...
has_codex_login_auth() {
  local auth_file
  for auth_file in \
    "$HOME/.codex/auth.json" \
    "${XDG_CONFIG_HOME:-$HOME/.config}/codex/auth.json"; do
...
if [ "$CTI_RUNTIME" = "codex" ] || [ "$CTI_RUNTIME" = "auto" ]; then
  ...
  # from config.env or current env, or a local auth.json created by `codex login`.
  CODEX_AUTH=1
  if [ -n "$(get_config CTI_CODEX_API_KEY)" ] || [ -n "$(get_config CODEX_API_KEY)" ] || [ -n "$(get_config OPENAI_API_KEY)" ] || \
     [ -n "${CTI_CODEX_API_KEY:-}" ] || [ -n "${CODEX_API_KEY:-}" ] || [ -n "${OPENAI_API_KEY:-}" ]; then
    CODEX_AUTH=0
  elif has_codex_login_auth; then
    CODEX_AUTH=0
  fi
...
ESBUILD_STATUS="missing"
ESBUILD_DETAIL=""
if [ -d "$SKILL_DIR/node_modules/esbuild" ]; then
  ESBUILD_CHECK_LOG="$(mktemp)"
  if (cd "$SKILL_DIR" && node --input-type=module -e "import('esbuild').then(async (esbuild) => { await esbuild.build({ stdin: { contents: 'console.log(1)', resolveDir: process.cwd(), sourcefile: 'doctor-esbuild-check.ts' }, write: false, bundle: true, platform: 'node', format: 'esm' }); process.exit(0); }).catch((error) => { console.error(error?.message || String(error)); process.exit(1); })") >"$ESBUILD_CHECK_LOG" 2>&1; then
    ESBUILD_STATUS="ok"
  elif grep -q "You installed esbuild for another platform than the one you're currently using." "$ESBUILD_CHECK_LOG"; then
    ESBUILD_STATUS="foreign"
  else
    ESBUILD_STATUS="broken"
    ESBUILD_DETAIL=$(tr '\n' ' ' <"$ESBUILD_CHECK_LOG" | sed 's/  */ /g' | cut -c1-160)
  fi
  rm -f "$ESBUILD_CHECK_LOG"
fi
...
if [ -f "$DAEMON_MJS" ]; then
  ...
  if [ -d "$SKILL_DIR/node_modules/claude-to-im/src" ]; then
    STALE_DEP=$(find "$SKILL_DIR/node_modules/claude-to-im/src" -name '*.ts' -newer "$DAEMON_MJS" 2>/dev/null | head -1)
  fi
fi

case "$ESBUILD_STATUS" in
  ok)
    check "esbuild matches current platform" 0
    ;;
  foreign)
    if [ -f "$DAEMON_MJS" ] && [ -z "$STALE_SRC" ] && [ -z "$STALE_DEP" ]; then
      check "esbuild matches current platform (foreign-platform install detected, but current dist/daemon.mjs can still run)" 0
    else
      check "esbuild matches current platform (foreign-platform install detected — reinstall deps on this OS before rebuilding)" 1
    fi
    ;;
  broken)
    check "esbuild matches current platform (failed to load: ${ESBUILD_DETAIL:-unknown error})" 1
    ;;
  *)
    check "esbuild package installed (not found — run 'npm install' in $SKILL_DIR)" 1
    ;;
esac

if [ -f "$DAEMON_MJS" ]; then
  if [ -z "$STALE_SRC" ] && [ -z "$STALE_DEP" ]; then
    check "dist/daemon.mjs is up to date" 0
  elif [ -n "$STALE_DEP" ]; then
    if [ "$ESBUILD_STATUS" = "foreign" ]; then
      check "dist/daemon.mjs is stale (bundled dependency changed, but rebuild is blocked by foreign-platform esbuild)" 1
    else
      check "dist/daemon.mjs is stale (bundled dependency changed, run 'npm run build')" 1
    fi
  elif [ "$ESBUILD_STATUS" = "foreign" ]; then
    check "dist/daemon.mjs is stale (src changed, but rebuild is blocked by foreign-platform esbuild)" 1
  else
...
if [ -f "$CONFIG_FILE" ]; then
  case "$(uname -s)" in
    MINGW*|MSYS*|CYGWIN*)
      check "config.env permissions check skipped on Windows" 0
      ;;
    Darwin)
...
if [ "$(uname -s)" = "Linux" ]; then
  if systemd_user_available; then
    check "Linux supervisor path: systemd --user available" 0
  elif is_wsl; then
    if command -v tmux >/dev/null 2>&1; then
      check "Linux supervisor path: WSL tmux fallback available (install-service unavailable until WSL systemd is enabled)" 0
    else
      check "Linux supervisor path: WSL without systemd --user or tmux (install tmux or enable WSL systemd for reliable background startup)" 1
    fi
  else
    check "Linux supervisor path: systemd --user unavailable, will use setsid/nohup fallback" 0
  fi
fi
...
if [ -f "$PID_FILE" ]; then
  ...
  check "PID file consistency (tmux session $TMUX_SESSION is running; waiting for bridge.pid or using tmux fallback)" 0
else
...
if [ "$FAIL" -gt 0 ]; then
  ...
  echo "  esbuild platform mismatch → cd $SKILL_DIR && npm install   # run from this OS (Windows vs WSL)"
  ...
  echo "  WSL without systemd   → prefer tmux-backed start; enable /etc/wsl.conf [boot] systemd=true only if you need install-service"
fi
