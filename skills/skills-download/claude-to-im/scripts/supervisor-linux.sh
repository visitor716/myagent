#!/usr/bin/env bash
# Linux supervisor — prefer systemd --user when installed, then tmux on WSL/non-systemd sessions,
# and finally setsid/nohup.
...
tmux_session_exists() {
  tmux_available && tmux has-session -t "$TMUX_SESSION" >/dev/null 2>&1
}

tmux_fallback_preferred() {
  is_wsl && ! systemd_user_available && tmux_available
}
...
start_tmux_session() {
  local node_path tmux_command
  node_path="$(command -v node)"
  printf -v tmux_command 'cd %q && exec %q %q >> %q 2>&1' \
    "$SKILL_DIR" \
    "$node_path" \
    "$SKILL_DIR/dist/daemon.mjs" \
    "$LOG_FILE"
...
supervisor_start() {
  ...
  if tmux_fallback_preferred; then
    if start_tmux_session; then
      return 0
    fi
    echo "tmux fallback start failed; falling back to setsid/nohup." >&2
  fi
...
supervisor_status_extra() {
  if supervisor_is_managed; then
    ...
    echo "Bridge is running in tmux session ($TMUX_SESSION)"
    echo "Linux supervisor mode: tmux fallback"
  elif is_wsl && ! systemd_user_available; then
    echo "systemd --user is not available in this WSL session."
    echo "Start uses tmux when available, otherwise setsid/nohup."
  fi
}
