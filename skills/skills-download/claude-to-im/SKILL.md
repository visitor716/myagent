...
description: |
  ...
  Subcommands: setup, start, stop, status, logs, reconfigure, doctor, install-service, uninstall-service.
...
argument-hint: "setup | start | stop | status | logs [N] | reconfigure | doctor | install-service | uninstall-service"
allowed-tools:
  - Bash
  - Read
  - Write
...
| `install service`, `enable autostart`, `开机自启`, `自启动`, `自启` | install-service |
| `uninstall service`, `disable autostart`, `关闭自启动`, `取消自启` | uninstall-service |
...
Linux start behavior:
- Prefer `systemd --user` when the unit is installed and the current session exposes a user bus.
- In WSL or other Linux sessions where `systemd --user` is unavailable, prefer the skill's `tmux` fallback when `tmux` exists.
- Fall back to `setsid`/`nohup` only when neither of the above is available.
...
- If the shell reports `systemd user session not available in WSL` or similar, do not keep retrying `install-service`; use the normal `start` path and let the script pick `tmux`, or enable WSL systemd explicitly before using `install-service`.

### `stop`

Run: `bash "SKILL_DIR/scripts/daemon.sh" stop`

If stop reports that the current shell could not signal the PID, treat that as a permission / sandbox issue rather than a clean stop. Rerun with elevated permissions instead of deleting runtime files manually.

### `status`

Run: `bash "SKILL_DIR/scripts/daemon.sh" status`

If status says the process could not be verified but `status.json` still reports `running=true`, treat that as an ambiguous sandbox / permission case. Confirm with recent logs or rerun status with elevated permissions.
...
- WSL session has no `systemd --user` bus → prefer the script's `tmux` fallback for `start`; only use `install-service` after enabling WSL systemd and restarting WSL
- `tmux` / `systemctl --user` access is denied inside a sandboxed agent shell → rerun the command with elevated permissions, then verify with logs if PID probing is blocked
...
Run: `bash "SKILL_DIR/scripts/daemon.sh" install-service`

Use this when the user wants login-time auto-start or crash recovery. On Linux this installs and enables a `systemd --user` unit. On Windows, the platform daemon helper already supports service installation. If the platform reports that install-service is unsupported, tell the user that this build does not yet provide a native service installer there.

WSL note:
- `install-service` only works when the distro is running with `systemd=true` in `/etc/wsl.conf` and WSL has been fully restarted (`wsl --shutdown` from Windows).
- If `systemctl --user` fails with `Failed to connect to bus` or `systemd user session not available in WSL2`, stop trying to install the service in that session. Use the normal `start` path instead; the script will prefer `tmux` when available.

### `uninstall-service`

Run: `bash "SKILL_DIR/scripts/daemon.sh" uninstall-service`

Use this when the user wants to remove login-time auto-start. After uninstalling, remind the user they can still run the bridge manually with `start`.
...
- The daemon runs as a background Node.js process managed by platform supervisor (launchd on macOS, `systemd --user` on Linux when available, `tmux` preferred in WSL / non-systemd Linux agent sessions, then `setsid`/`nohup` fallback, WinSW/NSSM on Windows).
