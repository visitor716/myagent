...
Options:
  ...
  --include-active            Allow action on worktrees with active local processes
...
Safety:
  ...
  - Active worktrees are skipped unless --include-active is set.
EOF
}
...
scan_worker() {
  local worker="$1"
  local path branch head counts behind ahead status_output dirty_count active_count
  ...
  active_count="$(count_active_processes_under "$path")"
  safe_worker="$(sanitize "$worker")"
  ...
  elif [[ "$active_count" != "0" && "$INCLUDE_ACTIVE" != "true" ]]; then
    class="active-keep"
    action="keep"
  elif [[ "$ahead" != "0" ]]; then
  ...
  printf '%-4s branch=%-8s head=%-8s base_only=%-3s worker_only=%-3s dirty=%-3s active=%-3s match=%-3s mismatch=%-3s artifacts=%-3s class=%s action=%s\n' \
    "$worker" "$branch" "$head" "$behind" "$ahead" "$dirty_count" "$active_count" \
  ...
  if contains_worker "$worker"; then
    if [[ "$active_count" != "0" && "$INCLUDE_ACTIVE" != "true" ]]; then
      echo "REFUSE $worker: active process count $active_count; use --include-active only after stopping/reviewing it"
    elif [[ "$ahead" != "0" ]]; then
...
main() {
  ...
  echo
  printf 'worker branch   head     base worker dirty active match mismatch artifacts class/action\n'
