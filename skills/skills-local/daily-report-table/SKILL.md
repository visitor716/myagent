...
4. The script should:
   - generate `日报表格-{date}.xlsx` by default
   - only generate Markdown/TSV outputs when explicitly requested with `--write-mode all`, `--write-mode notes`, or `--write-mode tsv`
...
- Default save mode generates only the XLSX workbook for the current run: `日报表格-{date}.xlsx`. It does not create `.md` or `.tsv` files unless a write mode is explicitly requested.
- Use `--format tsv` when the user wants easier spreadsheet pasting.
- Use `--write-mode tsv` when the user wants to save only TSV tables and skip Markdown notes.
...
- `--write-mode none` skips Markdown, TSV, and企业微信日报 HTML writes. Pair it with `--chart-copy` when you only need the图表粘贴列。
