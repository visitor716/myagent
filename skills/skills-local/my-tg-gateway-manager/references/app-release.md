...
If public `TG_WEBAPP_URL` fails before sending, use the Runtime recovery lane in `my-tg-gateway-manager`: recreate the tunnel, update `.env`, restart Gateway, then send the fresh button.
