...
Use this lane to recover `tg-agent-gateway` when Telegram appears alive in code but the phone UI, command menu, WebApp, or public URL does not respond. Treat "menu does nothing" as a runtime-path problem until logs prove callback routing is the culprit.
...
2. Run the bundled runtime snapshot:
...
If manual `getMe` works with proxy but Gateway has no proxy env, fix `scripts/restart-gateway.sh` so tmux receives the proxy exports. Do not put tokens in logs. It is acceptable to log proxy host/port without credentials.

### Mobile WebApp URL And Tunnel Target

- `TG_WEBAPP_URL` is the button target for new `/start` messages; old Telegram messages keep their old inline-button URL.
- Telegram clients can keep an already-open WebApp popup/iframe. After quick-tunnel rotation, stale iframe URLs can still point at the old `*.trycloudflare.com` URL and show `530 The origin has been unregistered from Argo Tunnel`.
- Desktop Chrome can prove the tunnel serves HTML, but it is not enough. Outside Telegram it may show `Not running in Telegram WebApp`, and it does not prove what URL the Telegram popup is loading.
- Production WebApp is served by Gateway on `WEBAPP_PORT` / `localhost:3000`; Vite dev is `5173`.
...
If local `3000/health`, public HTML, and asset downloads are healthy but the phone still shows an endless loading state, suspect a stale Telegram WebApp popup/WebView or stale runtime asset state before assuming Cloudflare is down. Build the WebApp, restart Gateway, and send a fresh button with a timestamp query parameter to bypass WebView cache.
...
For Telegram Web proof, use `wsl-windows-chrome` first, select the logged-in Telegram Web tab, close old WebApp popups, click the newest fresh button, then inspect iframe URLs:
...
Healthy Telegram proof: the WebApp iframe URL starts with the current `TG_WEBAPP_URL`, the snapshot shows app content such as `当前项目` / bottom navigation, and the page does not contain Cloudflare 530 text.

### Telegraf Launch Semantics

- In long polling mode, `bot.launch()` does not normally resolve; it waits in the polling loop.
- Do not block downstream startup forever by awaiting `bot.launch()` for manager and then starting webapp/workers afterward.
...
- Use both `bot.telegram.setMyCommands(commands)` and `bot.telegram.setChatMenuButton({ menuButton: { type: 'commands' } })`.
...
Standardize main-menu return buttons on `back_to_menu`. Keep compatibility routes for old in-flight buttons, such as `back_to_main`, until old Telegram messages age out.

### Main-Menu Send Lock

If logs show `Main menu request ignored because another send is in flight`, the first `/menu` send is still pending. Treat this as a stuck Telegram API call, not user error. Wrap main-menu `reply`, `editMessageText`, and fallback sends with a bounded timeout so the in-flight lock is released and later `/menu` attempts can recover.

### Worker Bot Token Conflicts

`launchAsBot` workers must use their own configured token via `getBotToken(config.tokenEnv)`. Never launch worker bots with `MANAGER_BOT_TOKEN`; that can create 409 polling conflicts and make the manager appear unresponsive.
...
3. If the tunnel and production assets are healthy but the phone is still loading, build the WebApp, restart Gateway, send a cache-busted timestamped button, then retest before editing React.
...
- Telegram WebApp iframe points at the current `TG_WEBAPP_URL`, not an old `*.trycloudflare.com` URL, and the snapshot shows app UI such as `当前项目`.
