---
name: skill-scout
description: Use this ECC-derived skill in Codex before creating, forking, or installing a skill. Search local Codex/agent skill roots, marketplace/GitHub sources, and web references to avoid duplicating an existing workflow.
metadata:
  origin: community
---
...
```bash
find ~/.codex/skills ~/.agents/skills ~/.claude/skills -maxdepth 2 -name SKILL.md 2>/dev/null | grep -iE "keyword|synonym"
