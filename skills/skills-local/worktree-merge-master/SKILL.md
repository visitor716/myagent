...
- **Merge-to-master lane**: merge ready worker commits into local `master`, verify, restart Gateway/WebApp, and send the latest `/app` entry to the bot for phone self-test.
...
- **Completed dirty worker lane**: review stopped-output dirty worker
  worktrees as completed task results; accept the best patch into `master` or
...
3. Send the latest `/app` WebApp release entry to the Telegram bot.
4. Tell the user to phone self-test from the bot's fresh App button.
5. Do not push yet unless the user explicitly says `push`, `finalize`, or
   otherwise asks to publish the self-tested commit.

After the user accepts the phone self-test, finish the push/sync lane:

1. Push `master` with an ordinary fast-forward push.
...
Do not use `TG_GATEWAY_SKIP_RELEASE_NOTIFY=1` in this lane unless the user
explicitly asks to skip bot notification. A merge that changes local `master`
should produce a fresh App entry for phone self-test.
...
- no busy or unknown local activity is detected, unless `--include-active` is set
- the worker branch is ahead of `master`
- the worker branch is not behind `master`

It skips dirty, busy, unchanged, behind, diverged, missing, or invalid worktrees
and reports why. A tmux/Codex/Claude pane whose cwd remains in the worktree is
only `occupied`; if its captured output stays unchanged during the settle
window, it is not `busy` and does not block a clean branch from being merged.
Unobservable local activity is still treated as busy/unknown.
...
When a dirty worker has stopped output (`busy=0`) and appears to contain a
completed task result, do not stop at "dirty worktree". Treat it as a review
candidate:

1. Capture the worker's final pane output and diff stat.
2. Compare overlapping candidates, for example `cc2` versus `cx1`, and choose
   the most complete accepted patch.
3. Apply the accepted patch in a temporary `master` worktree, run verification,
   then create a Lore-format commit and fast-forward local `master`.
4. For rejected or superseded variants, preserve evidence and stash with
   `audit_dirty_worktrees.sh --stash-discarded "<workers>"`, then fast-forward
   those worktrees to `master`.
5. Restart Gateway/WebApp and send the latest App release notification for
   phone self-test. Do not push until the user accepts the phone test.
...
5. Continue the local merge-to-master lane: restart Gateway/WebApp with
   `bash scripts/restart-gateway.sh`, confirm the App release notification was
   sent or deliberately skipped by state dedupe, and tell the user to phone
   self-test before pushing. Close the completed worker tmux session only after
...
- `keep-review`: dirty content differs from `master`, has unique commits, or has
  busy/unknown activity. Preserve it until the work is reviewed or the worker is
  stopped or the pane has gone quiet.
...
`git stash push -u`, and then `git merge --ff-only master`. It refuses busy or
unknown activity unless `--include-active` is explicitly provided and refuses
branches with unique worker commits. It never resets, cleans, force-pushes, or
deletes a branch.
...
7. If the user asks to push worker refs after phone acceptance, push only the
   active branch set and verify local/remote counts are `0 0`.
...
7. when the base is `master`, runs `bash scripts/restart-gateway.sh`, which
   restarts the Gateway/WebApp and sends the latest App release notification to
   the bot unless the state file says the same commit was already notified
...
# Force-include worktrees even when local activity is busy or unobservable.
bash scripts/merge_ready_worktrees.sh --dry-run --include-active

# Tune stopped-output detection. The default samples tmux pane output for 3s.
bash scripts/merge_ready_worktrees.sh --dry-run --activity-settle-seconds 5

# Diagnostic only: merge without sending the App notification.
bash scripts/merge_ready_worktrees.sh --apply --no-release-notify
```
...
- no automatic push before phone self-test; push only when the user explicitly
  asks to publish/finalize the accepted local `master`
