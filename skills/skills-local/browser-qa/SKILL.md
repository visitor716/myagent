---
name: browser-qa
description: Use this ECC-derived skill in Codex for browser QA, visual testing, and UI interaction verification after frontend changes or deployments. Prefer the repo's existing Playwright tooling or the local wsl-windows-chrome skill for logged-in Windows Chrome automation.
metadata:
  origin: ECC
---
...
Use the repo's existing Playwright/Puppeteer tooling, or use the local `wsl-windows-chrome` skill when a logged-in Windows Chrome profile is required. Do not introduce browser MCP just for this workflow unless the user explicitly asks for MCP.
