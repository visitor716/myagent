...
### Optional: enable auto-start on Linux

If you want the bridge to start with your Linux user session, install the `systemd --user` unit:

```bash
bash scripts/daemon.sh install-service
bash scripts/daemon.sh start
```

This keeps start/stop/status under the same `daemon.sh` interface, but upgrades Linux from `setsid/nohup` fallback to first-class service management with restart-on-failure.
...
| `/claude-to-im install-service` | "enable autostart" / "install service" | Install and enable Linux user-session auto-start |
| `/claude-to-im uninstall-service` | "disable autostart" / "uninstall service" | Remove Linux user-session auto-start |
