...
- The user explicitly trusts Codex to manage local work autonomously on this
  machine. Keep the default local Codex posture high-autonomy:
  `approval_policy = "never"` and `sandbox_mode = "danger-full-access"` unless
  the user explicitly asks to reduce privileges.
...
- For browser automation from WSL, use the `wsl-windows-chrome` skill first.
  Do not add or use Chrome/Browser MCP for normal browser tasks unless the user
  explicitly asks for MCP.
- Browser work should attach to the dedicated Windows Chrome/Edge automation
  profile through the skill helper. If the dedicated Windows CDP endpoint is
  unavailable, stop with diagnostics instead of falling back to a fresh
  WSL/Linux browser.
- Long-term memory source files live in
...
- `scripts/codex_heartbeat.py` is the local heartbeat generator and should stay
