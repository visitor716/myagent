...
def main():
    ...
    sources = {
        'apps/cc-connect/start-cc-connect-hidden/start-cc-connect-hidden.py': root/'apps'/'cc-connect'/'start-cc-connect-hidden.py',
        'apps/cc-connect/restart-cc-connect-hidden/restart-cc-connect-hidden.py': root/'apps'/'cc-connect'/'restart-cc-connect-hidden.py',
        'apps/file-transfer/fileCopyUpMain/fileCopyUpMain.py': root/'apps'/'file-transfer'/'fileCopyUpMain.py',
        'apps/task-scheduler/CheckTask/CheckTask.py': root/'apps'/'task-scheduler'/'CheckTask.py',
        'apps/task-scheduler/CreateTaskWithSchtasks/CreateTaskWithSchtasks.py': root/'apps'/'task-scheduler'/'CreateTaskWithSchtasks.py',
    }
    for relative, target in sources.items():
        source=script_root/relative
        if source.exists():
